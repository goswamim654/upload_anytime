...
var Uploader = {
	...
	addEventsListeners: function() {
		...
		var fileselect = this.getById('userProfilePhoto');
	...
	upload: function (file) {
		...
		//xhr.open("POST", this.getById('profileImageupload').action);
		xhr.open("POST", "https://www.snaplava.com/live/public/api/v1/users/profile/upload-square-image");
			...
			if (xhr.status === 200) {
				...
				if(responseData.status == 1){
			        ...
			        $profilePic.croppie('bind', {
						...
						$("#circleImageLoaderContainer").addClass('hidden');
						$("#profileCorppingContainer").removeClass('hidden');
						$('#circle').croppie('bind')
						$("#profilePhoto").val(1);
						$("#profileSaveBtn").removeClass('disabled');
	...
	onUploadSuccessful: function (e, ident) {
		$("#profileCorppingContainer").addClass('hidden');
		$("#circleImageLoaderContainer").removeClass('hidden');
		this.markImageAsUploaded(ident);
		if($.inArray(ident, uploadedImageArr) == -1){
			uploadedImageArr.push(ident);
		}

		//$("#profileSaveBtn").removeClass('hidden');
		//$("#profileSaveBtn").removeClass('disabled');
	},
	...
	processFile: function(file) {
		...
		if($(".profileDragAndDropContainer").children('.validation-fields').length > 0) {
			$(".profileDragAndDropContainer").children('.validation-fields').remove();
		}
		$("#profilePhoto").val("");
		$("#profileSaveBtn").addClass('disabled');
		$("#headingLabelInDragDrop").addClass('hidden');
		$("#headingLabelInPreview").removeClass('hidden');
